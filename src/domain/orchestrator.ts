...
export const orchestrator = async (args: OrchestratorOptions) : Promise<ReportDetails> => {
  ...
  const {renamedFiles, deletedFiles, keywordFiles} : {renamedFiles: RenamedFile[], deletedFiles:File[], keywordFiles:any} = {renamedFiles:[], deletedFiles:[], keywordFiles:[]} //= await getSuspicsousFiles();
